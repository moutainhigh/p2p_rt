...
<body>
	...
	<div class="top-k">
		<div class="wydkxq-nr">
		<input type="hidden" value="tit_3_left9" id="curTitle">
...
<script type="text/javascript">
	...
	<c:forEach var="item" items="${BORROW_ALL_STATUS}"> 
		borrowStatus["${item.key}"] = "${item.value}";
	</c:forEach>
	function subForm() {
		...
		getPageTable(data, "${path }/iborrow/borrowStatusPage.do",
				generateInvestRecordsTable, "investRecordsTable", "9");
	}
	...
	function generateInvestRecordsTable(data) {
		...
		var htmlStr = '<tr>';
		htmlStr += "<td> <a href=${path}/invests/"+data.id+">" + data.borrowTitle + "</a></td>" + "<td>" + data.tName
				+ "</td>" + "<td>" + data.borrowSum + "</td>" + "<td>"
				+ data.annualInterestRate + "%</td>" + "<td>"
				+ data.borrowTimeLimit + "" + day + "</td>" + "<td>"
				+ data.borrowAddTime + "</td>" + "<td>" + jindu + "%</td>"
				+ "<td>"+borrowStatus[data.borrowStatus]+"</td>" + "<td>--</td>";
		htmlStr += "</tr>";
		return htmlStr;
	};
</script>
