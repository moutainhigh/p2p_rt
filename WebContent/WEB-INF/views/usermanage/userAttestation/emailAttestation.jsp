...
%>
	...
	<form id="pagerForm" onsubmit="return navTabSearch(this);"
		action="attestation/${PRE_PATH_VIEW }emailAttestation?right_id=${right_id}" method="post">
			...
			<div class="searchBar">
				<table class="searchContent">
					<tr>
						...
						<td>
						状态
							<select name="email" id="email"  onchange="change();">
									<option value="">全部</option>
								<c:forEach var="name" items="${USER_EMAIL_STATUS }">
									<option value="${name.key }">${name.value }</option>
								</c:forEach>
							</select>
							<input type="hidden" id="emailStatus" name="emailStatus" value="${searchParams.emailStatus }">
						</td>
						<td>
			...
			<div class="panelBar">
				<ul class="toolBar">
					<c:forEach items="${righSubtList}" var="rightsub">
						 <c:if test="${fn:contains(rightsub.moduleUrl,'attestation/v_toSaveEmailAttestation')}">
							<li>
								<a class="edit" href="${rightsub.moduleUrl}?supportID={userId}&right_id=${right_id}"
									target="dialog"  width="480" height="470" rel="jbsxBox"><span>${rightsub.moduleName}</span>
								</a>
							</li>
							<li class="line">
								line
		...
		function change(){
			//获取下拉框中的值
			 var email = $("#email").val();
			 if(email){
				 $("#emailStatus").val(email);
				 $("select[name='email']").val(email);
