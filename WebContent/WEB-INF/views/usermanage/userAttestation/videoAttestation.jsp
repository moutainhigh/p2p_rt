...
%>
	...
	<form id="pagerForm" onsubmit="return navTabSearch(this);"
		action="attestation/${PRE_PATH_VIEW }videoAttestation?right_id=${right_id}" method="post">
			...
			<div class="searchBar">
				<table class="searchContent">
					<tr>
						...
						<td>
						状态
							<select name="video" id="video"  onchange="change();">
									<option value="">全部</option>
								<c:forEach var="name" items="${USER_VIDEO_STATUS }">
									<option value="${name.key }">${name.value }</option>
								</c:forEach>
							</select>
							<input type="hidden" id="videoStatus" name="videoStatus" value="${searchParams.videoStatus }">
						</td>
						<td>
			...
			<div class="panelBar">
				<ul class="toolBar">
					<c:forEach items="${righSubtList}" var="rightsub">
						 <c:if test="${fn:contains(rightsub.moduleUrl,'attestation/v_toSaveVideoAttestation')}">
							<li>
		...
		function change(){
			//获取下拉框中的值
			 var video = $("#video").val();
			 if(video){
				 $("#videoStatus").val(video);
				 $("select[name='video']").val(video);
